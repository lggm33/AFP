...
class JobDetectorWorker(BaseWorker):
    ...
    def process_cycle(self):
        ...
        try:
            # Find EmailImportJobs ready to run
            ready_jobs = db.session.query(EmailImportJob)\
                ...
                .filter(
                    and_(
                        ...
                        EmailImportJob.status.in_(['completed', 'failed', 'idle'])
                    )
                ).all()
            ...
            for email_job in ready_jobs:
                try:
                    ...
                    # Schedule next run based on sync frequency
                    ...
                    self.logger.info(
                        f"Queued EmailImportJob {email_job.id} for Integration {email_job.integration_id} "
